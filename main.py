...
from history_utils import format_event_text
...
# Configure application logging once for the entire app
configure_logging()
...
async def get_session_history(
    ...
    """
    # Validate pagination
    # Cap max items to prevent server overload
    if limit > 100:
        limit = 100
    # Prevent negative or zero limits (invalid)
    if limit < 1:
        # Reset to default
        limit = 20
    # Prevent negative pages (Page 0 or -1 doesn't exist)
    if page < 1:
        # Reset to default
        page = 1

    # Offset determines how many records to skip.
